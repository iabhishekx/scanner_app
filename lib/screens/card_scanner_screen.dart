...
class _CardScannerScreenState extends State<CardScannerScreen>
  ...
  Future<void> _pickImage(ImageSource source) async {
    try {
      ...
      if (picked != null && mounted) {
        final provider =
            Provider.of<CardScanProvider>(context, listen: false);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Consumer<CardScanProvider>(
              ...
              SliverToBoxAdapter(
                child: Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      if (provider.state == CardScanState.idle)
                        _buildIdleState()
                      else if (provider.isLoading)
                        _buildLoadingState()
                      else if (provider.state == CardScanState.error)
                        _buildErrorState(provider)
                      else if (provider.state == CardScanState.success)
  ...
  SliverAppBar _buildAppBar(CardScanProvider provider) {
    return SliverAppBar(
      expandedHeight: 160,
      floating: false,
      pinned: true,
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
            'Running OCR and parsing details',
            style: TextStyle(
              color: AppTheme.textMuted,
              fontSize: 13,
            ),
  ...
  Widget _buildErrorState(CardScanProvider provider) {
    return Column(
      children: [
        const SizedBox(height: 20),
  ...
  Widget _buildResultState(CardScanProvider provider) {
    ...
    return Column(
      ...
      children: [
        ...
        InfoRow(
          icon: card.isValid ? Icons.verified : Icons.cancel,
          label: 'Luhn Validation',
          value: card.isValid ? 'Valid Card ✓' : 'Invalid Card Number ✗',
          iconColor: card.isValid ? AppTheme.success : AppTheme.error,
        ),
        if (card.cardNumber == null && card.expiryDate == null)
          Container(
  ...
  Widget _buildActionButtons(CardScanProvider provider) {
    return Column(
      children: [
        ScanActionButton(
